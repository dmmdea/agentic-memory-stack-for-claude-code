...
# --- 3b. v1.23.2/v1.23.3: the one-brain rule on the deploy path ---
# A replica's local mem0 is DORMANT by design (the offline watcher starts it only during an
# outage). The restart in step 5 would START it — the v1.23.1 deploy on the first demoted box did
# exactly that (started 10:38, stopped by hand at 10:41) — and then health-gate a store nobody
# reads. The import smoke below opens the Qdrant connection at import time, so it can never pass
# while the replica's stack is dormant either (v1.23.2 put this gate AFTER the smoke and both
# replicas stopped there with their files already synced). A dormant replica therefore gets a
# byte-compile of the synced modules here and stops; its real smoke is the /health/deep gate
# restore-replica runs when the watcher brings it up. A replica whose mem0 is UP (travel mode)
# goes through the smoke and the restart like a brain, on its loopback bind, but skips the
# retrieval-families gate, which judges the AUTHORITY's store.
if [ "${MEM0_ROLE:-brain}" = "replica" ]; then
    if ! systemctl --user is-active --quiet mem0.service; then
        _cerr="$(mktemp)"
        if ! "$APP_DIR/.venv/bin/python" -m py_compile "$APP_DIR"/*.py 2>"$_cerr"; then
            echo "==> BYTE-COMPILE FAILED for the synced server modules (files are on disk; nothing is running them):"
