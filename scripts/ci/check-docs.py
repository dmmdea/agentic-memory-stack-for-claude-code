...
README.md, CLAUDE.md resolves to a real file; (2) no operator-specific value appears
in docs/ (the repo's own canonical GitHub URL is sanctioned); (3) every ADR has valid
frontmatter (status/date; superseded_by iff Superseded). Exit 1 on any violation."""
import re, sys
...
PII = re.compile(r"(?i)\b(aorus|qube|daniel|readypep|eclipton|dojolife|danmar|peptidos|pepclick|intranet-pds)\b"
...
for f in DOC_FILES:
    ...
    for line_no, line in enumerate(text.splitlines(), 1):
        if any(s in line for s in SANCTIONED):
            continue
        if PII.search(line):
