...
The memory authority lives on the brain box (WSL: Qdrant + mem0 `:18791`). Over Tailscale it is reachable
...
`P:\memory-backups\<authority-host>` — derived at runtime from the authority URL, with
`-CloudBackupDir` as the override — lives on **pCloud's virtual drive**, which *streams* files from the cloud on
...
1. a **complete** set (manifest + episodic + history + qdrant, sharing a timestamp) on local disk
   (`D:\memory-backups\<authority-host>`, likewise derived; `-LocalBackupDir` overrides) wins;
...
- `on` **and** `off` refuse to run on the authority machine: `on` would overwrite the live brain with
  a stale snapshot, and `off` would stop and disable the live mem0/qdrant (both share the same WSL
  calls). The guard reads LOCAL receipts — `~/.mem0/role` being `brain`, or an authority URL that
  resolves to this machine — and fails closed when neither can be determined. It does **not** take a
  hostname parameter: the old `-AuthorityHost` defaulted to the operator-neutral literal
  `your-machine`, so the guard never fired on any real box, and deriving that hostname from the
  authority URL is equally vacuous on the brain (whose authority is loopback).
