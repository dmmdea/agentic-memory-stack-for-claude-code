...
- **Claude Code hooks** (in `~/.claude/settings.json`): `Stop`/`PreCompact` → `stop-extract.ps1` (Codex L1a extraction); `SessionStart` → storage-caps banner + daemon pre-warm + codex-shim pre-warm (opt-OUT via ~/.claude/state/codex-shim.disabled; it was opt-in behind a flag nothing created, which is why the judgment leg never ran); `UserPromptSubmit` → `mem0-hook-client.exe` (injects `[MEMORY CONTEXT]` via the resident daemon, fails open to the inline extractor); `PreToolUse` → `pre-tool-check.ps1`.
