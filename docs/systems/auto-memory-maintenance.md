...
**2026-09-10 revision — one compactor per PC, one judge attempt per store per night.** The per-store catch-up below ran at every session start with no cross-instance lock: four instances hit one store in the same second, the history repo's `index.lock` failed, 243 receipts landed in nine hours, and the judge was called 32 times on one store with every result rejected. Two changes, both interim relief ahead of the fleet-store design (ADR `fleet-store-sync-and-linux-authority`): (1) **GUARD 0** — a session-local named mutex (`Local\ams-memory-compact`) taken before anything else; a second instance logs `another compactor instance holds the per-PC lock` and exits 0 with no receipt, and the OS releases the mutex if the holder dies. (2) **One judge attempt per store per 20 h** — every receipt records `judge_called`, `Get-AmStoreRunHistory` exposes `LastJudgeUtc`, and a store whose judge was called inside the window still gets deterministic hygiene and the floors but no judge call; with nothing else to do it receipts `skipped-judge-attempted-today` (not productive, not a skip-streak entry). The window is 20 h, not 24, so a nightly a few minutes earlier than yesterday's does not find that attempt still "today". `-Force` bypasses it. The `-CatchUp` starved check applies the same window, so a session start no longer re-runs a store the judge already decided today. Found on the way: under pwsh 7 `ConvertFrom-Json` already yields a `[DateTime]` for a receipt's `ts`, and the `[string]` re-parse read it as local time — every receipt age was skewed by the UTC offset whenever the lib ran under pwsh 7; PS 5.1, the scheduled task's runtime, was unaffected. `ConvertTo-AmUtc` handles both.

**2026-09-08 revision — a live session can no longer starve a store past the sync limit, and starvation is reported.** A store was skipped as "live" on two consecutive nights — legitimately: a session had written memories 33 minutes before the 05:00 run — and grew from 19,032 to 26,424 B with no pass in between; the harness stopped syncing it and another session loaded a partial index. Nothing said so, for three separate reasons. The throttle stamp is **per run**, and a run that skipped this store still marked it because other stores reached a decision, so the SessionStart catch-up exited at every session start until the next 05:00, where the store was live again. `compactor-silent` keys on the receipts *file's* age, and a skip writes a receipt, so a store skipped every night looks alive to it. `compactor-unproductive` needs three consecutive bad receipts, and this store had `applied, skipped, skipped`. Four changes: (1) the liveness guard **escalates at the sync limit** — the quiet window drops from 30 to 5 minutes (a session between turns is not mid-write), and after two consecutive skips the run proceeds regardless and says so in the log and the receipt (`liveness_override`). A lost update is recoverable in one night from the snapshot and the dedangle pass; an index the harness refuses to load is broken for everyone. Under the limit the guard is unchanged. (2) Every receipt carries `skip_streak`. (3) The catch-up is **per store**: a fresh run stamp no longer ends it when a store above trigger has reached no decision in 24 h (`Get-AmStoreRunHistory`). (4) Lint raises **`compactor-starved`** — actionable, rendered by the heartbeat — on two consecutive skips above trigger, or one skip at the limit. Found and deliberately not changed: the write-time gate's matcher is `Write|Edit`, and this index is routinely written through Bash/python, which the gate never sees — it had never fired on any store; probed directly against an over-limit copy it works (29,630 → 24,906 B, receipted). Widening it to `Bash` would spawn `powershell.exe` on every shell call; that trade is the operator's.

**2026-09-06 revision — the nightly actually runs every night, and lines converge too.** An audit found two silent nights in one week and a store at 174 lines against the 200-line injection cutoff. Three changes: (1) the compactor's throttle is **12 h** (was 23 h), so an operator's daytime hand run can no longer make the 05:00 task declare itself too early; (2) the SessionStart spawner now launches `memory-compact.ps1 -CatchUp`, a debt gate that runs the nightly only when the newest receipt is older than 24 h (the case where the box was off at 05:00 and the scheduler's missed-start retry found no user logged on); (3) a **line floor**: when an index is over its line trigger after hygiene and the judge's decisions, the oldest pullable facts the judge did not migrate are migrated deterministically — same write-then-verify path, same blast cap, doctrine excluded — until the store is back at its line target (`line_floored` in the receipt). The judge is told about the line budget so its own MIGRATE choices come first. Also: a fact file with no frontmatter (a session re-created a migrated slug with only an addendum) is re-indexed with a hook made from its first line of prose instead of "recovered orphan; no description". And the doctrine rule gained a third test: an **attributed statement** (`Owner: …`, `Owner (CANONICAL): …`) is a standing order whatever verb follows — the first live line-floor run migrated one that was typed `project` and did not open with an imperative. The catch-up gate reads the **throttle stamp**, not the receipts file: a quiet night is productive and marks the stamp but writes no receipt by design.
...
| The judge was already called for this store in the last 20 h | judge withheld; hygiene and floors still run; `skipped-judge-attempted-today` when nothing else changed; throttle **not** marked |
| Another compactor instance is running on this PC | exit 0 at once, no receipt, one log line (GUARD 0 mutex) |
