...
- **Authority** — the brain's mem0 URL as seen by this box: loopback on the brain box itself, or the brain's remote URL on a replica box. Resolved as `MEM0_URL` env → `~/.mem0/authority-url` (the durable per-host file the installer writes) → loopback; the shim reads the result as `AUTHORITY_URL`. The Windows-side hooks (v1.23) read the same file from the Windows side (`~\.mem0\authority-url`, written by the installer beside `~\.mem0\role`) **first**, with `MEM0_URL` only as a fallback for a box that has no file — nothing in the stack writes that variable any more.
- **Local replica** — a dormant local mem0 + Qdrant on loopback (`127.0.0.1:18791`), started only during an outage, restored read-only from the newest snapshot set. Since v1.23.1 `restore-replica.ps1` refuses a set WSL cannot read (a streaming drive such as pCloud's `P:` is visible to Windows but not mounted in WSL — copy the set to a local drive first, which is what travel-mode's local cache is for) and refuses to report "restored" unless the point count equals the set's manifest.
...
The PowerShell hooks are a *separate* client that follows the same two rules since v1.23. **Reads** (the per-prompt bundle behind the `[MEMORY CONTEXT …]` block, daemon and inline path alike) try the authority and, on a replica whose authority does not answer, retry the same bundle against the dormant local store; the block's header then says `source=local-replica` instead of `source=authority:<host:port>`, so a session can always tell which box answered. **Writes** (the L1a extractor / C1 consolidator in [`../../scripts/windows/memory-common.ps1`](../../scripts/windows/memory-common.ps1)) never touch the replica: a connection failure or a retryable status appends an `add` op to the same Outbox the shim uses (`~/.mem0/outbox.jsonl` in WSL, the shim's record shape), and the replay driver delivers it with everything else. Only a deterministic 4xx (400/401/413/422) is set aside, in `~/.claude/state/mem0-post-poison.jsonl`, for a human. The dead-letter file (`mem0-post-failures.jsonl`) remains only as the fallback for the moment the Outbox itself is unreachable (WSL asleep, the `\\wsl.localhost` share not mounted) — a transient condition, so its drain on the next run re-posts the fact through the same path, which then queues it to the Outbox.
...
3. Acts only on a transition:
   ...
   - **`go_online`** — run the replay driver to drain the Outbox to the authority, then stop the local `qdrant`/`mem0` services and remove the vestigial `~/.mem0/travel.json` flag. (Nothing re-points the hooks: their authority file was never changed.)

The watcher resolves the authority carefully: an explicit `-Authority` wins; otherwise `MEM0_URL`, but **only if it is not a local/loopback/unspecified/malformed host** (a `Test-IsLocalUrl` guard). This guard dates from when `travel-mode.ps1 on` set a user-scope `MEM0_URL` pointing at the *replica* — without it, a fresh watcher tick inherited the replica as its "authority", probed it as healthy, and replayed the Outbox *into the disposable local store*, a One-Brain violation. Since v1.23 neither travel mode nor the watcher writes that variable (the hooks resolve the per-host file), and the guard stays as the belt against a stray value from anywhere else.
