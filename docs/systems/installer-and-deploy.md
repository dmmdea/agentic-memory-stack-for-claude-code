...
5. Enables **only** `qdrant.service`, `mem0.service`, `l10-audit.timer` and `ams-nightly.timer` (plus every `ams-step-*.service`, whose `WantedBy=` binds only once enabled); every per-job timer a previous install may have enabled is turned off (one chain, see below). Every home-relative path in a rendered unit is `%h`, never `/home/<tenant>`: the Linux user and the mem0 tenant differ on a native box. `__SECRETS_DIR__` is rendered into the step units and `CODEX_HOME=<secrets>/codex` into the drop-in. When `/etc/nftables.d/ams.nft` exists and passwordless `sudo` works, the bind belt is persisted as the root oneshot `ams-nft.service` (never `nftables.service`, whose conf would flush the iptables-nft tables). `--eval-root <dir>` (a checkout holding `eval/retrieval-drift/retrieval_drift.py`) and `--pcloud-dir <dir>` land in `stack.env` as `MEM0_EVAL_ROOT` / `MEM0_PCLOUD_DIR`. `--render-only <dir>` writes the resolved unit set and exits without touching `$HOME`; the test suite greps that set for `/mnt/c`, `cmd.exe`, `powershell.exe`, the DPAPI fetch and `/home/<tenant>`.
