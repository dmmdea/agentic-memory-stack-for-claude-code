...
2. Provisions Qdrant and the mem0 server exactly as the WSL-side installer does — the module list, the Qdrant version and the pip line are **read from `install/1-wsl-services.sh` at run time**, never copied (pinned by a test) — in their own venv (`python3.12`/`3.13` from PATH, else a `uv`-managed 3.12 bootstrapped into the client venv), warming the BM25 cache the unit expects.
