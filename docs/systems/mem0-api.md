...
Three more added by W3 (the alarm-mouth track — all informational, never flip `ok`; the fixed key names below are a cross-track contract consumed by the verifier, the dream heartbeat, and the session-banner digest):

- **`job_liveness`**: age of every nightly receipt the stack leaves behind. Shape: `{"role": "brain"|"replica"|null, "last_dream_age_h": h|null, "prune_age_h": h|null, "gather_age_h": h|null, "backup_manifest_age_h": h|null, "dedup_report_age_h": h|null, "morning_summary_age_h": h|null, "morning_summary_sections_48h": N|null, "error"?: "..."}`. Every field is independently fail-soft — a missing file/env yields `null` for that field plus a note in `error` while the rest still populate. `last_dream_age_h` derives from the dream stamp file's epoch **content**, never its mtime (a copy/restore changes mtime); `role` comes from env `MEM0_ROLE`, else the WSL `stack.env` receipt.
