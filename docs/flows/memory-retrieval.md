...
- **Per prompt** — every substantive human `UserPromptSubmit` fires one `POST /v1/context/bundle` (Channel 1, working memory). A trivial, rate-limited or machine-turn prompt (a background task notification) sends the checkpoint-only `POST /v1/episodes/checkpoint` instead.
...
- **Human prompts only (C10).** Claude Code raises `UserPromptSubmit` for background task notifications too, and before C10 the block rode about 91% of them. A prompt that starts with `<task-notification>` (after leading whitespace) is a **machine turn**: the client sends the checkpoint-only POST (the same path a trivial prompt takes) and renders nothing. A notification queued behind a running turn arrives with the same wrapper. The verdict is applied at three points that share one test corpus: the lib's `Test-MachineTurnPrompt` (daemon and inline paths), the compiled client, which walks the stdin JSON so that only the top-level `prompt` counts (it skips leading whitespace with no length cap and reads truncated stdin as human). The client withholds, and logs with its byte count, any block the daemon returns for a machine turn. It relays the inline fallback's output unchanged, because that child already decided with a real JSON parse, and the server (`hook_contract.is_machine_turn_prompt`: the checkpoint lands, the sections come back empty with `machine_turn: true`, no search runs). Peer messages from another session are not task notifications and keep the human path.
- **Hygiene on repeat content.** The `insight` tier is filtered out of this hot path server-side (insights are for deliberate recall). Within a session, the client does not repeat itself (C10). It records what each emitted block showed in a per-session state file (`~/.claude/state/mem0-injected-<session_id>.json`, holding only hashes of the rendered lines), and both the daemon and the inline path read that file. A memory line already shown in the session is dropped. The goals and frontier-questions sections render only when their content differs from what the session was last shown. If every admitted memory was already shown, the R2 rule applies and no block renders at all. A compaction resets the state: `stop-extract.ps1` clears it on `PreCompact` (synchronous, so no prompt can slip in between), and `mem0-hook-daemon-spawn.ps1` clears it again on `SessionStart` with source `compact` (or `clear`) as a backstop. `resume` keeps the state, because a resumed context still holds its blocks. The first prompt after a compaction therefore re-surfaces everything. None of this can fail silently, and no failure can leave content suppressed. Every reset also writes a compaction marker (`mem0-injected-<session_id>.compacted`) holding the reset time, and the reader ignores any state saved before it. Each save is stamped with its request's start time, captured before the bundle POST. So a save that was already in flight when the reset landed reads as stale, and the daemon's cache does not keep it. A reset that cannot delete the file overwrites it with an empty state; if that also fails, the marker alone invalidates the stale file. The 7-day sweep removes a marker only when its state is gone or was saved after it. A marker that still guards a surviving stale file is kept, and each marker removal is logged. Each reset logs its outcome: `absent`, `removed`, `truncated`, `invalidated` or `FAILED`. An unreadable state reads as empty and is logged, and a failed save is logged with its path and exception. The log is `~/.claude/logs/user-prompt-extract.log`, or `~/.mem0/hook-daemon.log` for lines written inside the resident daemon; each line starts with `C10 injection-state:`. If the logs directory is unwritable, the line goes to stderr, which Claude Code shows in the hook's error output. The daemon keeps each session's state in memory and reloads it only when the file's mtime, its length or the marker changes, so a warm prompt pays no file read. Dedupe only removes lines, so any block that is emitted is byte-identical to the pre-C10 render of the reduced bundle, and the caps, the gate and the R6 placement are unchanged. This replaces the daemon-only goals/questions blanking (HK-5) and its fixed 25-prompt refresh, which stood in for the compaction signal.
...
Retrieval is predominantly read-only. The one write on the hot path is the **episode checkpoint** side effect of the per-prompt bundle (an in-progress episode upsert; see [`./memory-capture.md`](./memory-capture.md)). Client-side, each emitted block updates the session's injection state (`~/.claude/state/mem0-injected-<session_id>.json`: 16-hex hashes of the memory lines shown plus a hash per goals/questions section, never memory text). A compaction deletes it, and files older than 7 days are swept. The bundle *derives* the `[MEMORY CONTEXT]` block injected above the prompt; the session-start précis *consumes and deletes* the PreCompact marker (`precompact-query.json`) whether fresh, stale, or corrupt. Client-side admission **rejections are logged** to `admission-rejected.jsonl` (memory id + reason) for observability; nothing else is persisted.
...
- **A machine turn gets no block** (C10). A prompt that starts with `<task-notification>` is gated by the lib, the compiled client and the server, which are held to one shared corpus.
- **A session is never shown the same memory line twice between compactions** (C10). Dedupe only removes content: an emitted block is byte-identical to the pre-C10 render of the reduced bundle, and R2 abstention covers the case where nothing novel is left.
...
- [`../../mem0-server/tests/test_machine_turn_gate.py`](../../mem0-server/tests/test_machine_turn_gate.py) — the server's machine-turn verdict over the shared corpus, and a bundle that serves a task notification nothing but its checkpoint.
- [`../../scripts/windows/tests/MachineTurnDedupe.Tests.ps1`](../../scripts/windows/tests/MachineTurnDedupe.Tests.ps1) — machine turns and in-session dedupe on the daemon path and the real inline fallback, the PreCompact and SessionStart-compact resets, and byte-parity of an emitted block. The compiled client's gate is covered in `HookClient.Tests.ps1`, and the corpus is [`../../scripts/windows/tests/fixtures/machine-turn-prompts.json`](../../scripts/windows/tests/fixtures/machine-turn-prompts.json).
