...
**The ephemera gate** is rule #2, equally binding, and exists by measurement (gate verdict 2026-08-19: ~22% of path-naming memories were point-in-time work pointers that should never have been stored — 81% of the staleness "problem" once hand-labelled). It drops work-state the next session's events supersede even when project-specific: one-off task/PR/branch/worktree pointers, session checkpoints and progress markers, temp/scratch locations (`AppData\Local\Temp`, scratchpads, Desktop exports, render dirs), and point-in-time counts ("the ledger had 19,883 lines"). Its test: *would this still be true AND worth retrieving in 30 days, by someone with no memory of this session?* The gate carries an explicit KEEP clause because the same hand-label found 23 VALID accused memories whose substance was durable and whose path was incidental. The boundary is stated as a decidable test, not a vibe: KEEP only when the sentence asserts a decision, rule, or measured finding that stays true without re-checking the path ("worktrees live under `D:\Dev\worktrees`" is a rule; "the egemma migration was rolled back because X" is a finding); a sentence whose **only** claim is "artifact X lives at path P" is a pointer and drops, however important the artifact feels in the moment.
